...
export class ChannelBillPlanRoutes{
          ...
          try {
            ...
            switch (value) {
              case 'CHANNELBILLPLAN_ADD':
                ...
                responseModelOfGroupDto = this.createChannelBillPlan(result["message"]);
                break;
              case 'CHANNELBILLPLAN_UPDATE':
                console.log("Inside CHANNELBILLPLAN_UPDATE Topic");
              //  responseModelOfGroupDto = this.updateChannelBillPlan(result["message"]);
                break;
              case 'CHANNELBILLPLAN_DELETE':
                console.log("Inside CHANNELBILLPLAN_DELETE Topic");
                responseModelOfGroupDto = this.deleteChannelBillPlan(result["message"]);
                break;
  ...
  @Put("/:id")
  async updateChannelBillPlan(@Param('id') id: number,@Body() body:RequestModel<ChannelBillPlanDto>): Promise<ResponseModel<ChannelBillPlanDto>> {  //requiestmodel<GroupDto></GroupDto>....Promise<ResponseModel<Grou[pDto>>]
    try {
      await console.log("Inside CreateProduct of controller....body id" + JSON.stringify(body));
      body.DataCollection.forEach((dto: ChannelBillPlanDto) => {
        dto.Id = id;
      })

...
import { SectionDailyEngagementDto, SectionDto, SectionInteractionReportDto, SectionPublicationReportDto, SectionWeeklyEngagementDto, TopCoursesDaywiseDto, TopCoursesDto } from '../../submodules/platform-3.0-Dtos/sectionDto';
...
export class SectionRoutes{
 ...
 async getCourseInteractionReport(@Param('pageSize') pageSize: number,@Param('pageNumber') pageNumber: number,@Req() req:Request): Promise<ResponseModel<SectionInteractionReportDto>>{
  ...
  // endpoint to get Top sections daily engagement Analytics
  @Get("/getDailyEngagement/:pageSize/:pageNumber")
  async getDailyEngagement(@Param('pageSize') pageSize: number,@Param('pageNumber') pageNumber: number,@Req() req:Request): Promise<ResponseModel<SectionDailyEngagementDto>>{
    try {
      console.log("getSectionDailyEngagement ......group by pageSize & pageNumber");
      let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
     //  requestModel.Filter.PageInfo.PageSize = pageSize;
     //  requestModel.Filter.PageInfo.PageNumber = pageNumber;
      let given_children_array = requestModel.Children;
     ...
     //applying query on retrieved data fields 
     let queryResult = await this.lessonFacade.genericRepository.query(`SELECT * from public.fn_get_top_course_daily_engagement(${communityId},${channelId},'${startDate}','${endDate}',${userId},${userRole},${pageNumber},${pageSize})`);     
     let final_result_updated = [];
     let result:ResponseModel<SectionDailyEngagementDto> = new ResponseModel("SampleInbuiltRequestGuid", null, ServiceOperationResultType.success, "200", null, null, null, null, null);
       
     queryResult.forEach((entity:any)=>{
         entity = objectMapper(entity,mapperDto.sectionDailyEngagementMapper); // mapping to camel case
...
async getPublishedSectionCount(@Req() request: Request){
 ...
 async getWeeklyEngagement(@Param('pageSize') pageSize: number,@Param('pageNumber') pageNumber: number,@Req() req:Request): Promise<ResponseModel<SectionWeeklyEngagementDto>>{
   try {
     console.log("getSectionsWeeklyEngagement ......group by pageSize & pageNumber");
     let requestModel: RequestModelQuery = JSON.parse(req.headers['requestmodel'].toString());
    //  requestModel.Filter.PageInfo.PageSize = pageSize;
    //  requestModel.Filter.PageInfo.PageNumber = pageNumber;
     let given_children_array = requestModel.Children;
     ...
     requestModel.Filter.Conditions.forEach((condition:Condition)=>{
      switch(condition.FieldName){
        ...
        queryResult.forEach((entity:any)=>{
            entity = objectMapper(entity,mapperDto.sectionWeeklyEngagementMapper); // mapping to camel case
